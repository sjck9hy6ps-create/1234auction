...
// 이 파일과 shared-villa.mjs가 예전에 쓰던 자료는 이런 개편 이전 상태라
// 오래되고 부정확했습니다. 다구(多區) 도시 표기도 shared.mjs 관례(예:
// '경기 수원 장안구', '시' 없음)를 그대로 따릅니다 — 수집·표시 전체에서
// 지역명 표기를 일치시켜야 하기 때문입니다.
export const LAWD_CODES = [
  ...
  // ⚠️ 2026-08: 부천시는 2024-01-01부로 원미구/소사구/오정구가 재설치되면서
  // 카카오맵 등 최신 행정경계 API가 더 이상 통합코드(41190)를 반환하지 않고
  // 구코드(41192/41194/41196)를 반환함 - 그런데 국토부 실거래 DB는 재설치 이전에
  // 수집한 데이터를 여전히 통합 명칭("경기 부천시")으로 저장해 두고 있어서, 구코드가
  // LAWD_CODES에 없으면 지역명 매칭에 실패해 house_trades/villa_trades 조회 자체가
  // 통째로 스킵됨(=지도에 매매기록 배지가 아예 안 뜸). 세 구 코드를 모두 같은 지역명에
  // 매핑해 기존에 쌓인 데이터를 그대로 찾을 수 있게 하고, 앞으로의 신규 수집도 이
  // 통합 명칭으로 계속 쌓이게 함(구 단위로 잘게 쪼갤 실익이 없음).
