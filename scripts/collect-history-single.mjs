import { LAWD_CODES, sleep, DELAY_MS, fetchMonthSingle, upsertSingle } from './shared-villa.mjs';
...
for (const month of targetMonths) {
  ...
  for (const { code, name } of LAWD_CODES) {
    const rows = await fetchMonthSingle(code, name, ym);
    monthRows.push(...rows);
    callCount++;
