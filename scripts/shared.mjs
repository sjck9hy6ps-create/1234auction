...
export const LAWD_CODES = [
  ...
  // ⚠️ 2026-09: 2026-07-01 인천 행정구역 개편(중구+동구→제물포구, 중구 영종/용유/무의
  // 8개동→영종구, 서구→서해구, 서구 검단 8개동→검단구) 반영. 이전엔 이 파일만 옛날 코드
  // (28110/인천중구, 28140/인천동구, 28260/인천서구)를 그대로 쓰고 있어서, 여기서 수집한
  // 아파트 매매 데이터가 계속 옛날 지역명으로 저장되고 "인천 제물포구" 등으로 조회하는
  // 지도 화면에는 안 보이는 문제가 있었음(shared-villa.mjs/lawd-codes.mjs는 이미 새
  // 코드로 맞춰져 있었는데 이 파일만 누락돼 있었음).
  ...
  // ⚠️ 2026-08: 부천시 2024-01-01 구 재설치(원미/소사/오정)로 카카오맵이 이제
  // 구코드를 반환함 - 옛 통합코드(41190) 대신 세 구코드를 같은 지역명에 매핑해서
  // 기존 DB 데이터(전부 "경기 부천시"로 저장돼 있음)를 계속 찾을 수 있게 함
