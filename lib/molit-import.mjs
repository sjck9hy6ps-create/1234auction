/* ════════════════════════════════════════════════════════════
   ...
   - ⚠️ api/ 폴더 "밖의" scripts/ 폴더에 둬야 함 (get-house.js/search-complex.js가
     scripts/lawd-codes.mjs를 쓰는 것과 동일한 위치). api/lib/ 처럼 api/ 폴더
     안에 두면 Vercel Hobby 플랜의 서버리스 함수 12개 한도에 걸릴 위험이 있음.
...
export const LAWD_CODES = [
  ...
  // (shared.mjs/lawd-codes.mjs/shared-villa.mjs와 반드시 같이 맞춰야 함)
  { code: '41192', name: '경기 부천시' }, { code: '41194', name: '경기 부천시' },
  { code: '41196', name: '경기 부천시' },
...
const BUILDERS = {
  house_trades: (row, m) => ({
    ...
    // 2026-09 추가: CSV 원본에 이미 있던 "거래유형"(직거래/중개거래) 컬럼 - 지금까지 파싱 안 하고
    // 버리고 있었음. 없으면(옛 CSV 포맷 등) null로 남김 - "직거래 아님"이 아니라 "알 수 없음".
    dealing_type: row['거래유형'] || null,
  }),
  house_rent: (row, m) => ({
    region: m.region, dong: m.dong, danji: row['단지명'] || '',
