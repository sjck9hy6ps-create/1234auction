/* ════════════════════════════════════
   경매서류 교차분석: 등기부등본 · 매각물건명세서 · 전입세대확인서 ·
                     감정평가서 · 현황조사서 · 건축물대장 (최대 6종)
   - PDF를 Supabase Storage("registry-docs" 버킷)에 원본 그대로 영구 저장
   - Gemini에 PDF를 직접 첨부(inline_data)해서 서류들을 서로 대조하며
     건물 이력을 시간순 스토리로 설명 + 경매 매수 관점 리스크
     (임차인 대항력, 말소기준등기, 낙찰 후 인수/말소되는 등기, 유치권 점유흔적,
     위반건축물 여부 등) 분석
   - 6종 모두 필수는 아님 - 등기부등본만 올려도 분석되지만(구버전과 동일 동작),
     여러 서류를 함께 올릴수록 서로 다른 자료 출처를 교차검증할 수 있어서 특히
     "임차인이 실제로 전입돼 있는가", "매각물건명세서상 임차인현황과 전입세대확인서가
     일치하는가", "감정평가서·현황조사서에 나온 점유상태가 서로 맞는가" 같은 판단의
     신뢰도가 크게 올라감
   - ⚠️ Vercel 하드리밋(요청당 4.5MB)상 원본 파일 합계 3MB 제한은 그대로 유지되며,
     6종을 나눠서 여러 번 업로드하면 이 API는 매번 그 호출에 포함된 서류만 다시
     분석해서 이전 결과를 덮어씀(누적/병합 안 함) - 6종을 모두 반영한 최종 분석을
     원하면 마지막 호출 한 번에 6종을 함께 담아 보내야 함
...
const DOC_LABELS = {
  ...
  appraisal: '감정평가서(법원 감정평가서 - 유치권 점유 흔적·현장 사진·가격 산정 근거 등)',
  fieldSurvey: '현황조사서(집행관 현황조사서 - 실제 점유자·임차 여부 현장 확인 내용)',
  buildingRegister: '건축물대장(위반건축물 여부·사용승인일·면적·용도 등 공적 확인 자료)',
};
// ⚠️ Vercel 플랫폼 자체가 요청 하나당 4.5MB 하드 리밋을 걸어놓고 있어(코드로 못 늘림),
// 6종 서류를 다 올려도 원본 합계는 여전히 3MB 이내여야 함(index.html의 analyzeAuctionDocs
// 체크와 동일한 한도를 공유). 한 번에 다 못 올리면 여러 번 나눠 올리되, 이 API는 매번
// "새로 첨부된 서류들만" 다시 분석해서 이전 결과를 덮어쓰는 방식이라는 걸 프론트 안내문에
// 명시해둠(병합 로직은 없음 - 6종을 다 반영하려면 마지막 호출에 6종을 함께 담아야 함).
...
감정평가서가 첨부됐다면 현장 사진·참고사항에 유치권 점유 흔적이나 미사용승인·건축중단
같은 특이사항이 언급돼 있는지 확인하고, 현황조사서가 첨부됐다면 집행관이 현장에서 확인한
실제 점유자·임차 여부가 매각물건명세서·전입세대확인서 내용과 일치하는지 대조하세요.
건축물대장이 첨부됐다면 위반건축물 여부·사용승인일·실제 면적이 다른 서류(감정평가서,
매각물건명세서)에 적힌 내용과 일치하는지 확인하세요.
...
export default async function handler(req, res) {
  ...
  if (!docsInput || typeof docsInput !== 'object') {
    return res.status(400).json({ error: 'docs가 필요합니다 (등기부등본/매각물건명세서/전입세대확인서/감정평가서/현황조사서/건축물대장 중 최소 1개).' });
  }
