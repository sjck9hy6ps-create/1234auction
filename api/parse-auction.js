...
const SCHEMA_B = {
  ...
  properties: {
    ...
    // 문건/송달내역에 세무서·지자체가 "교부권자"로 표기되어 송달된 기록이 있는지 - 압류등기가
    // 없어도 체납세금이 존재할 수 있다는 사실상 유일한 1차 신호(정확한 금액은 알 수 없음).
    // 대항력 있는 임차인이 있는 물건에서 특히 중요함(세금의 법정기일이 임차인 확정일자보다
    // 빠르면 배당에서 세금이 우선해 임차인 미배당액=낙찰자 인수액이 늘어날 수 있음).
    taxClaimDelivery: { type: 'BOOLEAN' },
    taxClaimant: { type: 'STRING' }, // 예: "역삼세무서", "강남구청"
    taxClaimNote: { type: 'STRING' }, // 관련 원문 문구 그대로
...
- floor는 지번주소 뒤의 "N층" 표기에서 숫자만 뽑으세요. 지하층(예: "지1층")은 -1로 담으세요. 숫자 없이 "지층"만 있는 경우(지하가 한 층뿐인 건물의 관행적 표기, 예: "지층2호")도 동일하게 -1로 담으세요 - null로 남기면 층수 비교 필터가 아예 작동하지 않아 지하 물건이 고층 매물과 잘못 비교됩니다.
...
- taxClaimDelivery(교부권자 송달 여부)는 "문건/송달내역", "송달내역", "문건접수내역" 같은 섹션의 표에서
  수신자/대상 칸에 "교부권자"라는 표기와 함께 세무서·시청·구청·군청 등 과세관청 이름이 함께 나오면 true로
  하고, taxClaimant에 그 관청 이름(예: "역삼세무서", "강남구청")을, taxClaimNote에 해당 행의 원문 문구를
  그대로 담으세요. "교부청구"라는 단어가 다른 문맥(교부청구권자가 아닌 배당 관련 일반 서술 등) 없이
  단순히 언급만 된 경우에도 taxClaimDelivery는 true로 두되, 관청 이름이 특정되지 않으면 taxClaimant는
  null로 두세요. 그런 섹션 자체가 없거나 교부권자 표기가 전혀 없으면 taxClaimDelivery는 false로 하세요
  (판단 근거가 되는 섹션을 아예 찾을 수 없는 경우에만 null). 절대로 금액을 추정하지 마세요 - 이 필드들은
  "체납 세금이 존재할 가능성이 있다"는 신호일 뿐, 금액은 알 수 없다는 전제로 다룹니다.
